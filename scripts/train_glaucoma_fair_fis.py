...
parser.add_argument('--fair_scaling_beta', default=0.9, type=float)
parser.add_argument('--fair_scaling_level', default='individual', type=str, help='individual|group')
...
def train(model, criterion, optimizer, scaler, train_dataset_loader, epoch, total_iteration, 
                imbalanced_info=None, fair_scaling_level='individual', fair_scaling_beta=1., attr_index=0):
    global device
    ...
    for i, (input, target, attr) in enumerate(train_dataset_loader):
        ...
        with torch.cuda.amp.autocast():
            ...
            if pred.shape[1] == 1:
                ...
                # loss = ( (torch.exp(loss/fair_scaling_beta) / torch.exp(loss/fair_scaling_beta).sum())*loss.shape[0]*loss ).mean()
                loss = scale_losses(loss, attr=attr[attr_index], level=fair_scaling_level, fair_scaling_beta=fair_scaling_beta)
            ...
            # pred_prob = torch.sigmoid(pred.detach().squeeze(1))
            # pred_prob = F.softmax(pred.detach(), dim=1)
    ...
    # aucs, dpds, dprs, eods, eors = evalute_perf_by_attr(preds, gts, attrs)
    # print(aucs)
    # sys.exit()
...
if __name__ == '__main__':
    ...
    trn_havo_dataset = Harvard_Glaucoma_Fairness(os.path.join(args.data_dir, 'train'), modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type, needBalance=args.need_balance, dataset_proportion=args.dataset_proportion)
    tst_havo_dataset = Harvard_Glaucoma_Fairness(os.path.join(args.data_dir, args.subset_name), modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type)
    ...
    imb_info = Imbalanced_Info(beta=args.imbalance_beta, no_of_attr=len(samples_per_attr))
    ...
    #         with open(args.perf_file, 'w') as f:
    ...
    for epoch in range(start_epoch, args.epochs):
        train_loss, train_acc, train_auc, trn_preds, trn_gts, trn_attrs = train(model, criterion, optimizer, scaler, train_dataset_loader, epoch, total_iteration, 
                                                                                imbalanced_info=imb_info, fair_scaling_level=args.fair_scaling_level, fair_scaling_beta=args.fair_scaling_beta, attr_index=attribute_index)
        test_loss, test_acc, test_auc, tst_preds, tst_gts, tst_attrs = validation(model, criterion, optimizer, validation_dataset_loader, epoch, imbalanced_info=imb_info)
        scheduler.step()
