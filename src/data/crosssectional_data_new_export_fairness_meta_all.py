...
ratio_of_patients_for_test = 0.2
num_attr = 3
...
#     bscanPixelspacingDepth = float(row['bscanPixelspacingDepth'])
    ...
    try:
        ...
        if pid not in rnflt_maps.keys():
            ...
            rnflt_attrs[pid] = [[glaucoma, md, tds, octID, datadir, clockhours, race, male, row.to_dict()]]
            ilm_maps[pid] = [img_ilm2nd]
            upids.append(pid)
        else:
            rnflt_maps[pid].append(rnflt_map)
            rnflt_attrs[pid].append([glaucoma, md, tds, octID, datadir, clockhours, race, male, row.to_dict()])
...
for pid in upids[:test_patients]:
    ...
    for ind in indices:
        ...
        glaucoma, md, tds, octID, datadir, clockhours, race, male, raw_info_dict = rnflt_attrs[pid][ind]
