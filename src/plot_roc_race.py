...
#       project=expr_name,
#     )
# wandb.run.name = f"unlabeled+labeled"
# wandb.run.save()

# wandb.init(
#       project=expr_name,
#     )
# wandb.run.name = f"labeled"
# wandb.run.save()
...
# rand_indices = list(range(0, len(data_to_plot[0])))
...
for i, one_attr in enumerate(np.unique(attr_1).astype(int)):
    # cur_auc = auc_score(pred_1[attr_1 == one_attr], gt_1[attr_1 == one_attr])
...
if showLegend:
    ...
    # plt.legend(fontsize=25, bbox_to_anchor=(0, 1.02, 1, 0.2), loc="lower left",
    #             mode="expand", borderaxespad=0, ncol=1, frameon=False)
